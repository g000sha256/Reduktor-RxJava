...
class StoreTest {
    ...
    fun `side effect stream was complete - states stream not completed`() {
        ...
        val testSubscriber = PublishProcessor
            .create<String>()
            .createDefaultStoreFlowable(
                sideEffectsIterable = listOf(sideEffect)
            )
            .test()
    ...
    @Test
    fun `multiple subscription - first subscriber was disposed`() {
        // GIVEN
        val actionsPublishProcessor = PublishProcessor.create<String>()
        val statesFlowable = actionsPublishProcessor.createDefaultStoreFlowable()
        val testSubscriber1 = statesFlowable.test()
    ...
    @Test
    fun `multiple subscription - all subscribers were disposed`() {
        // GIVEN
        val actionsPublishProcessor = PublishProcessor.create<String>()
        val statesFlowable = actionsPublishProcessor.createDefaultStoreFlowable()
        val testSubscriber1 = statesFlowable.test()
    ...
    private fun Flowable<String>.createDefaultStoreFlowable(
        ...
        sideEffectsIterable: Iterable<SideEffect<String, String>> = emptyList(),
        scheduler: Scheduler = Schedulers.trampoline()
    ): Flowable<String> {
        return toStoreFlowable(state, reducer, sideEffectsIterable, scheduler, tag = null)
    }
